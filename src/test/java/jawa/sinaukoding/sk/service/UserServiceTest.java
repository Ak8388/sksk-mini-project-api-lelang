...
class UserServiceTest {
    ...
    void deleteUser_NotFound(){
        ...
        final User admin = userRepository.findById(1L).orElseThrow();
        final Authentication authentication = new Authentication(admin.id(), admin.role(), true);
        Mockito.when(userRepository.deleteUser(ArgumentMatchers.any(),ArgumentMatchers.any())).thenReturn(0L);

        Response<Object> response = userService.deletedResponse(authentication, Delete, null);
        Assertions.assertEquals("0601", response.code());
        Assertions.assertEquals("Gagal menghapus. Data sudah di hapus atau data tidak ditemukan", response.message());
    }    
}
